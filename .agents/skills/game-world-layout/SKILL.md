...
- **Short landscape phone:** no longer a target since 24 September 2026: phones are portrait only and landscape phones see a rotate screen (see docs/DESIGN.md). Earlier guidance, kept for tablets and small windows: leave the center full height for the board. Use side rails: back/name left; Menu, Others and time controls right. Place players, goals, hand and game actions in the remaining side-rail space without moving navigation. This is the exception to the bottom-hand layout. Verify all targets and source-edge coverage without board scrolling. Compact visual controls must remain usable; check rail overlaps with goals, confirmation and all extension actions.
